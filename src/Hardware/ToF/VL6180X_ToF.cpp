...
units::meter_t VL6180X_ToF::getRange() {
    ...
    if (!isMeasuring) {
        ...
        if ((status & 0x01) == 0x01) {
            ...
            FRC_ReportError(frc::err::Error, "VL6180X_ToF 0x4d Status Error {}", status);
        }
    }
    ...
    if (isMeasuring) {
        ...
        if ((readRegister(REG_RESULT_INTERRUPT_STATUS_GPIO) & 0x04) == 0x04) {
            ...
            FRC_ReportError(frc::err::Error, "VL6180X_ToF 0x4f Timed out reading measurement status");
            return 42_m;
        }
    }
...
uint8_t VL6180X_ToF::readRegister(uint16_t reg) {
    ...
    if (res) {
        FRC_ReportError(frc::err::Error, "VL6180X_ToF I2C::ReadOnly() timed out reading register {::#x}", reg);
    }
